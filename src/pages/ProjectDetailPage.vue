...
<template>
  <div v-if="project" class="min-h-screen bg-paper -mt-24 pt-24 md:-mt-28 md:pt-28 pb-24">
    ...
    <Teleport to="body">
      <div v-if="isLightboxOpen" class="fixed inset-0 z-[100] flex items-center justify-center bg-ink/95 backdrop-blur-md transition-opacity">
        ...
        <div class="relative max-h-[85vh] max-w-[85vw] mx-auto" @click.stop>
          ...
          <div class="absolute -bottom-16 left-1/2 -translate-x-1/2 text-small tracking-[0.3em] uppercase font-bold text-paper/70">
    ...
    <section v-reveal class="mx-auto max-w-[1600px] px-5 pt-6 md:px-8 lg:px-10">
      <div class="relative w-full aspect-[16/9] lg:aspect-[21/9] min-h-[400px] border border-line bg-muted overflow-hidden rounded-shell">
        ...
        <nav class="absolute top-6 left-6 md:top-8 md:left-10 z-20 flex items-center gap-2 text-label font-bold uppercase tracking-[0.2em] text-paper/70 drop-shadow-md">
        ...
        <div class="absolute bottom-6 right-6 md:bottom-10 md:right-10 z-10">
          <button @click="scrollToGallery" class="inline-flex items-center gap-3 rounded-full border border-paper/40 bg-ink/20 px-6 py-3 md:px-8 md:py-4 text-label font-bold uppercase tracking-[0.2em] text-paper shadow-sm transition-all duration-300 hover:bg-paper hover:text-ink hover:border-paper backdrop-blur-md">
            Zobrazit galerii
          </button>
        </div>
      </div>
    ...
    <div class="mx-auto max-w-[1600px] px-5 py-12 md:px-8 lg:px-10 flex flex-col lg:flex-row gap-16 relative items-start">
      ...
      <div class="flex-1 w-full min-w-0 flex flex-col gap-10 lg:gap-16">
        <div v-reveal>
          <p class="text-label font-bold uppercase tracking-[0.3em] text-ink/45">Detail projektu</p>
          <h1 class="mt-5 max-w-3xl text-display font-bold tracking-[-0.05em] text-ink leading-[0.9]">
            {{ project.name }}
          </h1>
          <div class="mt-10 flex flex-wrap gap-4 text-label font-bold uppercase tracking-[0.18em] text-ink/70">
      ...
      <aside v-reveal="200" class="w-full lg:w-[420px] shrink-0 lg:sticky top-32 z-10">
        <div class="rounded-3xl border border-ink bg-ink p-8 md:p-10 text-paper shadow-2xl">
          <p class="text-label font-bold uppercase tracking-[0.3em] text-paper/50">Hlasovat</p>
          <h2 class="mt-4 text-h2 font-bold tracking-[-0.04em]">Dejte svůj hlas tomuto projektu.</h2>
          <p class="mt-4 text-small leading-relaxed text-paper/72">
          ...
          <form class="mt-8 space-y-4" @submit.prevent="submitVote">
            <label class="block text-label font-bold uppercase tracking-[0.18em] text-paper/60" for="vote-email-1">
              E-mailová adresa
            </label>
            <input
              id="vote-email-1"
              ...
              class="w-full rounded-full border border-paper/20 bg-paper/10 px-6 py-4 text-body font-bold text-paper outline-none transition placeholder:text-paper/35 focus:border-paper"
              required
            />
            <button
              type="submit"
              class="w-full rounded-full border border-paper bg-paper px-6 py-5 text-label font-bold uppercase tracking-[0.2em] text-ink transition hover:bg-transparent hover:text-paper shadow-sm mt-4"
            >
              Hlasovat
            </button>
          </form>

          <p v-if="voteSubmitted" class="mt-5 rounded-soft border border-paper/20 bg-paper/10 px-5 py-4 text-small font-bold leading-relaxed text-paper/78">
    ...
    <section id="gallery" class="w-full border-y border-line bg-canvas py-16 lg:py-28 my-10">
      <div v-reveal class="mx-auto max-w-[1600px] px-5 md:px-8 lg:px-10">
        <h2 class="text-h2 font-bold tracking-tight text-ink mb-10">Fotogalerie</h2>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 auto-rows-[200px] md:auto-rows-[300px] grid-flow-row-dense">
           ...
           >
             ...
             <div class="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity duration-300 group-hover:opacity-100">
               <span class="rounded-full bg-paper/90 px-6 py-3 text-label font-bold uppercase tracking-[0.2em] text-ink shadow-lg">Zvětšit</span>
             </div>
           </div>
        </div>
      </div>
    ...
    <div class="mx-auto max-w-[1600px] px-5 py-12 md:px-8 lg:px-10 flex flex-col lg:flex-row gap-16 items-start">
      ...
      <div class="flex-1 w-full min-w-0 flex flex-col gap-20">
        ...
        <section v-reveal>
          <h2 class="text-h2 font-bold tracking-tight text-ink mb-12">Informace o projektu</h2>
          
          <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-16">
            
            <!-- Parametry -->
            <div>
              <h3 class="text-label font-bold uppercase tracking-[0.3em] text-ink/45 mb-6 border-b border-line pb-4">Parametry</h3>
              <ul class="space-y-4">
            ...
            <!-- Dopravní dostupnost -->
            <div>
              <h3 class="text-label font-bold uppercase tracking-[0.3em] text-ink/45 mb-6 border-b border-line pb-4">Dopravní dostupnost a vybavenost</h3>
              <ul class="space-y-4">
                <li class="flex justify-between items-center border-b border-line/50 pb-3">
            ...
            <div class="md:col-span-2 mt-4">
              <h3 class="text-label font-bold uppercase tracking-[0.3em] text-ink/45 mb-6 border-b border-line pb-4">Příslušenství</h3>
              <ul class="grid grid-cols-2 md:grid-cols-3 gap-6 text-ink/80">
                <li class="flex items-center gap-3">
                  ...
                  <span class="font-bold text-small text-ink">Garáže</span>
                </li>
                <li class="flex items-center gap-3">
                  ...
                  <span class="font-bold text-small text-ink">Parkovací stání</span>
                </li>
                <li class="flex items-center gap-3">
                  ...
                  <span class="font-bold text-small text-ink">Sklepní kóje / sklad</span>
                </li>
                <li class="flex items-center gap-3">
                  ...
                  <span class="font-bold text-small text-ink">Hřiště</span>
                </li>
                <li class="flex items-center gap-3">
                  ...
                  <span class="font-bold text-small text-ink">Recepce</span>
                </li>
                <li class="flex items-center gap-3">
                  ...
                  <span class="font-bold text-small text-ink">Výtah</span>
                </li>
              </ul>
            </div>
          </div>
        ...
        <section v-reveal="100" class="rounded-[2.5rem] border border-line bg-canvas p-8 md:p-12 mb-10 relative overflow-hidden">
          ...
          <p class="text-label font-bold uppercase tracking-[0.3em] text-ink/45 mb-8 relative z-10">Developer</p>
          
          <div class="flex flex-col sm:flex-row sm:items-center gap-6 mb-10 relative z-10">
            <div class="flex items-center justify-center h-24 w-24 rounded-2xl bg-line/50 text-ink/80 font-bold text-h2 uppercase shrink-0 border border-ink/10 shadow-sm">
              {{ project.developer.logo }}
            </div>
            <h2 class="text-h1 font-bold tracking-tight text-ink">{{ project.developer.name }}</h2>
          </div>

          <div class="grid lg:grid-cols-[1.2fr_1fr] gap-12 relative z-10">
            <div>
              <h3 class="text-lead font-bold mb-4 text-ink">O developerovi</h3>
              <p class="text-ink/80 leading-relaxed text-small md:text-body">
                {{ project.developer.about }}
              </p>
            </div>
            <div class="lg:border-l lg:border-line/50 lg:pl-12">
              <h3 class="text-lead font-bold mb-4 text-ink">Kontakt</h3>
              <ul class="space-y-4 text-ink/80 text-small md:text-body">
                <li class="font-bold text-ink text-body">{{ project.developer.address }}</li>
                <li class="flex items-center gap-3">
                  <span class="w-20 font-bold uppercase text-label tracking-widest text-ink/70">Telefon</span> 
                  <span class="font-bold">{{ project.developer.phone }}</span>
                </li>
                <li class="flex items-center gap-3">
                  <span class="w-20 font-bold uppercase text-label tracking-widest text-ink/70">E-mail</span> 
                  <span class="font-bold">{{ project.developer.email }}</span>
                </li>
                <li class="flex items-center gap-3">
                  <span class="w-20 font-bold uppercase text-label tracking-widest text-ink/70">Web</span> 
                  <a :href="project.developer.web" target="_blank" class="font-bold text-ink underline transition hover:text-ink/70">{{ project.developer.web }}</a>
                </li>
              </ul>
            </div>
          </div>
      ...
      <aside class="w-full lg:w-[420px] shrink-0 lg:sticky top-32 z-10">
        <div class="rounded-3xl border border-ink bg-ink p-8 md:p-10 text-paper shadow-2xl">
          ...
          <form class="mt-8 space-y-4" @submit.prevent="submitVote">
            <label class="block text-label font-bold uppercase tracking-[0.18em] text-paper/60" for="vote-email-2">
              E-mailová adresa
            </label>
            <input
              id="vote-email-2"
    ...
    <section class="mx-auto max-w-[1600px] px-5 pt-20 md:px-8 lg:px-10 border-t border-line mt-10">
      <div v-reveal class="mb-12">
        <p class="text-label uppercase tracking-[0.32em] text-secondary font-bold">Podobné projekty</p>
        <h2 class="mt-5 text-h1 font-medium tracking-[-0.04em] text-ink">
  ...
  <section v-else class="mx-auto max-w-[1600px] px-5 py-20 md:px-8 lg:px-10 lg:py-28 -mt-24 pt-44 md:-mt-28 bg-paper">
    <div class="rounded-card border border-line bg-paper p-10">
      <p class="text-label uppercase tracking-[0.3em] text-secondary font-bold">Detail projektu</p>
      <h1 class="mt-5 text-h1 font-bold tracking-[-0.04em] text-ink">Tento projekt nebyl nalezen.</h1>
      <p class="mt-5 max-w-2xl text-body leading-7 text-ink/70">
        Cesta sice existuje, ale tento záznam chybí v aktuálních datech užšího výběru.
      </p>
      <RouterLink
        to="/projekty"
        class="mt-8 inline-flex rounded-full border border-ink bg-ink px-8 py-4 text-label font-bold uppercase tracking-[0.2em] text-paper transition hover:-translate-y-1 shadow-md"
      >
        Zpět na projekty
      </RouterLink>
    </div>
